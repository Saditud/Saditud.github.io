...
const ArrayParti = [];
const divElement = document.getElementById("brick");
...
class Particule {
    constructor(x, y, size = Math.random() * 7 + 1, weight = Math.random() * 3 + 1, directionX = -1) {
        this.x = x;
        this.y = y;
        this.size = size;
        this.weight = weight;
...
function init() {
    while (ArrayParti.length !== 0)
        ArrayParti.pop();
    for (let i = 0; i < 100; i++)
        ArrayParti.push(new Particule(Math.random() * canvas.width, Math.random() * canvas.height));
}
function animate() {
